...
static inline Tensor::Dim GetGatherIndexDim(const gather_params& params) {
    std::cout << "GetGatherIndexDim()" << std::endl;
    switch (params.axis) {
    case GatherAxis::BATCH:
        std::cout << "    : Batch " << std::endl;
        return params.inputs[1].Batch();
    case GatherAxis::FEATURE:
        std::cout << "    : Feature " << std::endl;
        return params.inputs[1].Feature();
    case GatherAxis::W:
        std::cout << "    : W " << std::endl;
        return params.inputs[1].W();
    case GatherAxis::Z:
        std::cout << "    : Z " << std::endl;
        return params.inputs[1].Z();
    case GatherAxis::Y:
        std::cout << "    : Y " << std::endl;
        return params.inputs[1].Y();
    case GatherAxis::X:
        std::cout << "    : X " << std::endl;
        return params.inputs[1].X();
    default:
...
static inline std::string GetGatherMaxIndexDim(const gather_params& params) {
    ...
    std::cout << "GetGatherMaxIndexDim() : " << std::endl;
    std::cout << "    - index_dim.v : " << index_dim.v << std::endl;
...
static std::string GetDictionaryIndexOrder(const gather_params& params, size_t axis) {
    std::cout << "GetDictionaryIndexOrder() : axis : " << axis << std::endl;
...
static std::string GetIndicesIdxOrder(const gather_params& params, size_t axis, int64_t batch_dim) {
    std::cout << "GetIndicesIdxOrder() : " << std::endl;
    std::cout << "    - axis      : " << axis << std::endl;
    std::cout << "    - batch_dim : " << batch_dim << std::endl;
    std::cout << "    - dict_dim  : " << params.inputs[0].GetDims().size() << std::endl;
    std::cout << "    - index_dim : " << params.inputs[1].GetDims().size() << std::endl;
    std::cout << "    - index_dim : " << params.inputs[1].Dimentions() << std::endl;
    std::vector<std::string> idx_order;

    if ((axis == (size_t)batch_dim) && (axis > 1) && (params.inputs[1].GetDims().size() > 4)) {
        idx_order = GetFinalIndexOrder(params.inputs[1].GetDims().size());
    } else {
    ...
    std::cout << "    - idx_order : " << GetOrderString(idx_order) << std::endl;
    return GetOrderString(idx_order);
}
